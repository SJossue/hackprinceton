...
def run() -> list[dict]:
    ...
    for a in alerts:
        if a.suggested_action and a.suggested_action["type"] == "send_text":
            # Draft only — no real SMS. The demo value is the warm,
            # Claude-written text that WOULD go out to the caregiver;
            # actually sending is left to whatever delivery integration
            # a production deployment chooses (Twilio, Signal, email,
            # the user's own SMS client). Ambient display + dashboard
            # still surface the draft so judges see the Eragon output.
